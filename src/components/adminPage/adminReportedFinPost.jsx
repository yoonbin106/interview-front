  ...
  return (
    <TableContainer component={Paper} className={styles.reportedFinPostTableContainer}>
      <Table sx={{ minWidth: 400 }} aria-label="custom pagination table">
        ...
        <TableBody>
          {(rowsPerPage > 0
            ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) // 현재 페이지의 게시글만 보여줌
            : rows
          ).map((row) => (
            <TableRow key={row.id}>
...
// ReportedFinPost 컴포넌트: 신고 처리 완료된 게시글을 관리하는 페이지 컴포넌트
const AdminReportedFinPost = () => {
    // 하드코딩된 신고 처리 완료된 게시글 데이터
    ...
    const [filteredReportedFinPost, setFilteredReportedFinPost] = useState(reportedfinpost); // 필터링된 게시글 상태
    const [statusFilter, setStatusFilter] = useState(''); // 상태 필터 상태
    const [page, setPage] = useState(0); // 현재 페이지 상태
    const [rowsPerPage, setRowsPerPage] = useState(10); // 페이지당 행 수 상태
    ...
    const totalPages = Math.ceil(filteredReportedFinPost.length / rowsPerPage); // 전체 페이지 수 계산
                ...
                <div className={styles.reportedFinPostMainContainer}>
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <h2 className={styles.reportedFinPostTitle}>신고처리 완료된 게시글</h2>
